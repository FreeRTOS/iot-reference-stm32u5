...
class OtaProject:
    ...
    ecdsa_signer_certificate_public_key: str = "CHANGEME"
    # exmaple
    # "-----BEGIN PUBLIC KEY-----\n"\
    ...
    custom_wifi_credential_path: Path = Path(
        src_root + "/Common/config/kvstore_config.h"
    )
    ...
    custom_client_credential_keys_path: Path = Path(
        src_root + "/Common/config/ota_config.h"
    )
    ...
    custom_core_mqtt_config_path: Path = Path(
        src_root + "/Common/config/core_mqtt_config.h"
    )
    ...
    custom_ota_task_path: Path = Path(
        src_root + "/Projects/b_u585i_iot02a_ntz/Src/ota/ota_update_task.c"
    )
    ...
    custom_freertos_config_path: Path = Path(
        src_root + "/Common/config/FreeRTOSConfig.h"
    )
